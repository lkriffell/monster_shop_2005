...
RSpec.describe 'Cart show' do
  describe 'When I have added items to my cart' do
    describe 'and visit my cart path' do
      ...
      it 'there is a button to decrease the item next to each item' do
        visit "/cart"
      ...
      it 'I can decrease individual items from my cart' do
        visit "/cart"
        ...
        expect(page).not_to have_css("#cart-item-#{@tire.id}")
        expect(page).to have_css("#cart-item-#{@pencil.id}")
        expect(page).to have_css("#cart-item-#{@paper.id}")
      end

      it "can NOT decrease more items then there are inventory" do
        visit "/cart"
